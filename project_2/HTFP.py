...
import helpers.models as models  # Using the added HierarchicalTransformerFP model
...
def save_results(config, history, best_val_acc, test_acc, conf_matrix, id2label, results_dir):
    ...
    model_name = f"htfp_d{config['d_model']}_h{config['n_heads']}_l{config['num_layers']}_dr{config['dropout']}"
    if config['version'] == 2:
        model_name += "_v2"
        if config['cross_level_attn']:
            model_name += "_cross"
...
def train_and_evaluate(config, train_loader, val_loader, test_loader, device, id2label, results_dir, early_stopping_patience=5):
    ...
    # Create HierarchicalTransformerFP model
    if config['version'] == 1:
        model = models.HierarchicalTransformerFP(
        ...
        model = models.HierarchicalTransformerFPV2(
            n_classes=len(id2label),
            n_mfcc=config['n_mfcc'],
            d_model=config['d_model'], 
            n_heads=config['n_heads'],
            ...
            dim_ff=config['dim_ff'],
            dropout=config['dropout'],
            cross_level_attn=config['cross_level_attn'],
            fusion_method=config['fusion_method']
        ).to(device)
...
def main():
    parser = argparse.ArgumentParser(description='Speech Commands Classification with Hierarchical Transformer Feature Pyramid')
    ...
    parser.add_argument('--results_dir', type=str, default='results/htfp',
                      help='Directory to save results')
    
    # Model parameters
    parser.add_argument('--version', type=int, default=2, choices=[1, 2],
                      help='HTFP version to use (1 or 2)')
    parser.add_argument('--d_model', type=int, default=128,
                      help='Model dimension')
    parser.add_argument('--n_heads', type=int, default=4,
                      help='Number of attention heads')
    parser.add_argument('--num_layers', type=int, default=6,
                      help='Number of transformer layers (total across all levels)')
    parser.add_argument('--dim_ff', type=int, default=256,
                      help='Dimension of feed-forward network')
    parser.add_argument('--dropout', type=float, default=0.1,
                      help='Dropout rate')
    parser.add_argument('--fusion_method', type=str, default='concat',
                      choices=['add', 'concat', 'weighted', 'gated', 'hierarchical'],
                      help='Method to fuse hierarchical representations')
    parser.add_argument('--cross_level_attn', action='store_true',
                      help='Enable cross-level attention (only for v2)')
    ...
    config = {
        ...
        'cross_level_attn': args.cross_level_attn,
    ...
    if args.search:
        ...
        base_configs = [
            ...
            {'version': 1, 'd_model': 128, 'n_heads': 4, 'num_layers': 6, 'dim_ff': 256, 'fusion_method': 'add', 'cross_level_attn': False},
            {'version': 1, 'd_model': 128, 'n_heads': 4, 'num_layers': 6, 'dim_ff': 256, 'fusion_method': 'concat', 'cross_level_attn': False},
            {'version': 1, 'd_model': 128, 'n_heads': 4, 'num_layers': 6, 'dim_ff': 256, 'fusion_method': 'weighted', 'cross_level_attn': False},
            {'version': 2, 'd_model': 128, 'n_heads': 4, 'num_layers': 6, 'dim_ff': 256, 'fusion_method': 'concat', 'cross_level_attn': False},
            {'version': 2, 'd_model': 128, 'n_heads': 4, 'num_layers': 6, 'dim_ff': 256, 'fusion_method': 'gated', 'cross_level_attn': False},
            {'version': 2, 'd_model': 128, 'n_heads': 4, 'num_layers': 6, 'dim_ff': 256, 'fusion_method': 'hierarchical', 'cross_level_attn': False},
            {'version': 2, 'd_model': 128, 'n_heads': 4, 'num_layers': 6, 'dim_ff': 256, 'fusion_method': 'concat', 'cross_level_attn': True},
            
            # Small models
            {'version': 2, 'd_model': 64, 'n_heads': 2, 'num_layers': 3, 'dim_ff': 128, 'fusion_method': 'concat', 'cross_level_attn': False},
            {'version': 2, 'd_model': 96, 'n_heads': 3, 'num_layers': 3, 'dim_ff': 192, 'fusion_method': 'concat', 'cross_level_attn': False},
            
            # Varying dimensions and heads
            {'version': 2, 'd_model': 128, 'n_heads': 2, 'num_layers': 6, 'dim_ff': 256, 'fusion_method': 'concat', 'cross_level_attn': False},
            {'version': 2, 'd_model': 128, 'n_heads': 8, 'num_layers': 6, 'dim_ff': 256, 'fusion_method': 'concat', 'cross_level_attn': False},
            
            # Varying layers
            {'version': 2, 'd_model': 128, 'n_heads': 4, 'num_layers': 3, 'dim_ff': 256, 'fusion_method': 'concat', 'cross_level_attn': False},
            {'version': 2, 'd_model': 128, 'n_heads': 4, 'num_layers': 9, 'dim_ff': 256, 'fusion_method': 'concat', 'cross_level_attn': False},
            
            # Large models
            {'version': 2, 'd_model': 192, 'n_heads': 6, 'num_layers': 6, 'dim_ff': 384, 'fusion_method': 'concat', 'cross_level_attn': False},
            {'version': 2, 'd_model': 256, 'n_heads': 8, 'num_layers': 6, 'dim_ff': 512, 'fusion_method': 'concat', 'cross_level_attn': False},
        ]
        ...
        for i, cfg in enumerate(configs):
            ...
            print(f"  Version: {cfg['version']}, d_model: {cfg['d_model']}, n_heads: {cfg['n_heads']}, "
                  ...
                  f"cross_level_attn: {cfg['cross_level_attn']}, dropout: {cfg['dropout']}")
        ...
        print(f"  cross_level_attn: {best_config['cross_level_attn']}")
        ...
        larger_config['num_layers'] = min(best_config['num_layers'] + 3, 12)  # Add 3 layers, max 12
